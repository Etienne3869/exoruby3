...
def number_journalists (journalists)
   puts "counting : #{journalists.count}"
end


def handle_min(journalists)
    puts "Quel est le journaliste qui à le nom le plus court ?"
   handle_min = journalists.sort_by{|journalists| journalists.length}[0]
   puts handle_min
end

def number_journalists_5 (journalists)
    puts "Quels sont les journalistes avec 6 caractères dans leurs noms ?"
    journalists_5 = journalists.select {|journalists| journalists.length == 6}
    puts journalists_5
    puts "counting : #{journalists_5.count}"
 end

 def number_journalists_maj (journalists)
    puts "Combien commence par une majuscule ?"
    journalists_maj = journalists.grep(/@[A-Z]/).length
    puts journalists_maj
 end

 def handle_alpha (journalists)
   puts j_alpha = journalists.sort_by{|journalists| journalists.downcase}
   puts j_alpha
 end

def handle_long(journalists)
   puts long = journalists.sort_by(&:length)
   puts long
end

def handle_epencer(journalists)
    puts epenser = journalists.find_index("@epenser")
end

def handle_boucle(journalists)
   handle_max = journalists.max_by{|journalists| journalists.length}
    ...
    handle_max_length.times do |i|
      list_size = journalists.select{|journalists| journalists.length == i}
          if list_size != []
          puts "liste avec #{i - 1} caractères : #{list_size}"
          puts " "
      end
   end
end 
   
handle_boucle(journalists)